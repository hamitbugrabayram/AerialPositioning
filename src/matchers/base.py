...
class BaseMatcher(ABC):
    ...
    def match(
        ...
        Returns:
            Dict[str, Any]: A dictionary containing match results.
        """
    ...
    def _create_empty_result(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: A dictionary with empty arrays and failure status.
        """
        return {
            "mkpts0": np.array([]),
            "mkpts1": np.array([]),
    ...
    def estimate_homography(
        ...
        Returns:
            Tuple[Optional[np.ndarray], np.ndarray]: A tuple of
                (3x3 homography matrix or None, boolean inlier mask).
        """
        if len(mkpts0) < 4:
