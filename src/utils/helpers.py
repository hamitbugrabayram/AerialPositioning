...
# Import TileSystem for Mercator projection
...
def latlon_to_pixel(
    lat: float, lon: float, map_metadata: Dict[str, Any], map_shape: Tuple[int, int]
) -> Optional[np.ndarray]:
...
def calculate_predicted_gps(
    ...
    """Convert normalized map coordinates to GPS coordinates using Mercator projection.
    ...
    try:
        # Use Mercator projection if TileSystem is available and we have Level info
        ...
        # Fallback to linear interpolation (less accurate for latitude)
...
def _calculate_gps_mercator(
    ...
    """Calculate GPS using Mercator projection (Bing Maps TileSystem).

    This function converts normalized pixel coordinates to lat/lon using
    the proper Web Mercator projection that Bing Maps tiles use.
    ...
    # Get the NW corner's global pixel coordinates
    nw_pixel_x, nw_pixel_y = TileSystem.latlong_to_pixelXY(nw_lat, nw_lon, level)

    # Convert normalized center to local pixel coordinates
    map_h, map_w = map_shape[:2]
    center_x_norm, center_y_norm = normalized_center_xy
    local_pixel_x = center_x_norm * map_w
    local_pixel_y = center_y_norm * map_h

    # Convert local pixel to global pixel coordinates
    global_pixel_x = nw_pixel_x + local_pixel_x
    global_pixel_y = nw_pixel_y + local_pixel_y

    # Convert global pixel back to lat/lon using Mercator inverse
    pred_lat, pred_lon = TileSystem.pixelXY_to_latlong(
...
def calculate_location_and_error(
    ...
    try:
        ...
        if not (-0.5 <= norm_x <= 1.5 and -0.5 <= norm_y <= 1.5):
            ...
            print(
                f"Warning: Normalized center ({norm_x:.3f}, {norm_y:.3f}) "
