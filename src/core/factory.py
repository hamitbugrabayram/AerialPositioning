...
class PipelineFactory:
    ...
    Attributes:
        PIPELINE_REGISTRY (Dict[str, Tuple[str, str]]): Mapping of matcher type
            to module and class names.
    """
    ...
    @classmethod
    def create(cls, config: LocalizationConfig) -> Any:
        """Creates a matcher pipeline based on the provided configuration.
        Args:
            config: Localization configuration object.
        Returns:
        ...
        if matcher_type not in cls.PIPELINE_REGISTRY:
            ...
            raise ValueError(
                f"Unsupported matcher: '{matcher_type}'. " f"Supported: {supported}"
            )
