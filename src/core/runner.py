"""Localization orchestration logic for aerial position estimation.
This module contains the LocalizationRunner class which coordinates the
loading of metadata, preprocessing of images, execution of matching algorithms,
and calculation of localization accuracy.
"""
import time
...
class LocalizationRunner:
    """Orchestrates the visual localization process.
    This class manages the lifecycle of a localization run, including
    initialization of matchers, data loading, result computation, and reporting.
    Attributes:
        config (LocalizationConfig): Parsed localization configuration.
        pipeline (Optional[Any]): Initialized matcher pipeline.
        preprocessor (Optional[Any]): Image preprocessor instance.
        query_df (Optional[pd.DataFrame]): Metadata for query images.
        map_df (Optional[pd.DataFrame]): Metadata for satellite tiles.
        output_dir (Optional[Path]): Directory where results are stored.
    """
    ...
    def _load_helpers(self) -> bool:
        ...
        try:
            ...
            return False
    def run(self) -> None:
        """Executes the complete localization pipeline."""
    ...
    def _setup_output_directory(self) -> None:
        ...
        paths = self.config.data_paths
        base_dir = Path(paths["output_dir"])
    ...
    def _process_queries(self) -> List[QueryResult]:
        """Iterates over all query images and performs localization.
        Returns:
            A list of QueryResult objects containing estimated positions.
        """
        results = []
    ...
    def _filter_relevant_maps(
        ...
        for idx, map_row in map_df.iterrows():
            try:
                ...
                dist = self._haversine_distance(float(q_lat), float(q_lon), m_lat, m_lon)
    ...
    def _process_single_query(
        ...
        """Handles matching and localization for a single query image.
        Args:
    ...
    def _write_statistics_file(
        ...
        """Writes statistics to a localization_stats.txt file."""
